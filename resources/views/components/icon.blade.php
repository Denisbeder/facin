...
@php
    if (is_numeric($size)) {
        $size = !empty($size) ? $size : '4';
        $size = sprintf('h-%s w-%s', $size, $size);
    }
@endphp

@if(str()->of($name ?? null)->isNotEmpty())    
    @if($name !== 'spinner')        
        <i data-feather="{{ $name }}" {{ $attributes->merge(['class' => $size]) }}></i>
    @else
        <svg {{ $attributes->merge(['class' => trim($size . ' animate-spin')]) }}viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
